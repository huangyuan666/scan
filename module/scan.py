# -- coding: utf-8 --
import threading, socket,argparse,time,os,queue
from module import printc
from module import queue
from module import argparse

#扫描常用端口
PortList=[21,22,23,25,31,42,53,67,68,69,79,80,99,102,109,135,137,138,139,143,161,389,443,445,456,513,554,635,636,902,903,912,913,993,1001,1011,1024,1080,1170,1234,1245,1433,1502,1536,1537,1538,1539,1540,1542,1543,1544,1547,1548,1549,1801,2500,2504,3306,3389,3443,4444,5040,6000,6942,7680,7702,7720,7739,7777,7778,7779,7780,7807,7831,7833,8080,8307,8800,9015,9075,9081,9086,9087,9095,9144,9156,12051,13223,14367,14601,14610,14611,14612,14613,14614,14615,14616,14617,14618,14619,14620,14621,21440,21441,28317,35432,63342,65000]
#单IP扫描线程个数
nThread = 30
#线程锁
lock = threading.Lock()
...
#扫描特定主机
def scan_specific_hosts(ip_addr):
    sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    sock.settimeout(Timeout)
    address = (ip_addr, 80)
    try:
